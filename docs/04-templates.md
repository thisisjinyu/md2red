...
| 变体 | 用在什么内容 | 触发信号 | 源 recipe |
| --- | --- | --- | --- |
| **L-LEDGER 重型账本** | 原因 / 雷区 / 代价，每条带后果 | 条目 ≤6 且每条有「后果/代价」 | M08 |
| **L-CHECK 轻型清单** | 法则 / 习惯 / 技巧，可执行 | 每条是 do/don't，无重后果，可较多条 | M05 |
| **L-DATA 数据条目** | 每条带数字 / 研究数据 | 条目自带 stat | S09 / S11 |
| **L-LONG 长清单分屏** | N 很大，一屏放不下 | 条目 >6~7 | M05 压缩 + 分屏 |
| **L-STACK 堆叠+符号** | 标签 + 子句 + 标记 | 条目 = label + sub | S11 |
...
| L-CHECK 轻型 | HE-0001 6个晨间仪式 / WL-0002 甜品生存法则 | 每条是可执行习惯/法则，无重后果 |
...
> 同为「编号清单体」，HE-0001 走 L-CHECK 轻型、WL-0001 走 L-LEDGER 重型——这就是「不固定死模版」。

## 7. 跑通顺序

1. 先实现 **L-LEDGER + L-CHECK** 两个变体（证明「同形态多版式」）
2. 写选择依据 rubric → 拿 **HE-0001**（习惯/可执行 → L-CHECK 轻型 + 期号封面 + 小结尾页）走语义判断跑通 Extract → Classify → Route → Fill → Render
